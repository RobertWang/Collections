> 本文由 [简悦 SimpRead](http://ksria.com/simpread/) 转码， 原文地址 [mp.weixin.qq.com](https://mp.weixin.qq.com/s/Bw258KrzHDGt_Fe1itTvrw)

本文的整个推导过程顺着以下过程, 从 assumption 开始, 最终达到 protocol:

![](https://mmbiz.qpic.cn/mmbiz_jpg/ziaRl59xuuY8a4UBYuYuGTLibuyUsJGKMV1O7aibL02GHECUibkpFIsgPzdz92kQMyqTux1v8uht0x0CbUXhOBtdcw/640?wx_fmt=jpeg)
...
从我们要解决的问题出发: 实现一个 **分布式** 的, 强一致的 **存储** 系统. 存储系统是存储信息的, 我们首先给出**信息**和**分布式存储**的定义:
...
具体来说, 对某个信息的`读`操作, 每次得到的内容应该都是唯一的, 确定的. 这个定义将贯穿本文, 作为我们设计一致性协议的最根本的公理.

分布式存储
-----

*   **存储系统**可以看做一个可以根据外部命令 (Cmd) 改变系统状态 (State) 的东西. 例如一个 key-value 存储, `set x=1` 或 `set x=y+1` 都可以看做一个 Cmd.
...
在这个系统中, **分布式** 的特性要求 writer 只需能联系到一个 quorum 就可以完成一个**信息**的写入, 即, 实现 **quorum-write**. 而 reader 只需要联系到一个 quorum 就可以确定的读取到 **信息**, 即实现 **quorum-read**.

因为 writer 写入的信息 reader 必须能读到, 所以任意 2 个 quorum 必须有交集:

![](https://mmbiz.qpic.cn/mmbiz_svg/CJ35Z2cnZA1DyTGhNG3RdcSEI9sI90H68PwSvHnyAvWQDg0QFh4po0iazSBmCXL3uK49L3OLEZ6dS79A5IjKY9tZx8JltJudS/640?wx_fmt=svg)
...
![](https://mmbiz.qpic.cn/mmbiz_jpg/ziaRl59xuuY8a4UBYuYuGTLibuyUsJGKMVPfiam7EldzJIFcLPOu08xndzStThYcNTLxIheAxMGb45jwwTTdibiasTA/640?wx_fmt=jpeg)
...
这也就是一个记录变更操作 (Cmd) 的日志(log), 或称为 write-ahead-log(WAL). 而系统的 State 也由 WAL 唯一定义的. 在一个典型的 WAL + State Machine 的系统中(例如 leveldb ), WAL 决定了系统状态(State), 如这 3 条 log: `[set x=1, set x=2, set y=3]`. 而平常人们口中的 State Machine, 仅仅是负责最终将整个系统的状态呈现为一个 application 方便使用的形式, 即一般的 HashMap 的形式: `{x=2, y=3}`.
...
State 的 **全序关系** 来表示 commit 的有效性, 但到目前为止, State 本身是一个操作日志, 也就是一个 list, list 之间只有一个**偏序关系**, 即包含关系. 互不包含的 2 个 list 无法确定大小关系.

> 例如, 如果在 2 个节点上分别读到 2 个 log: `[x, y, z]` 和 `[x, y, w]`, 无法确认哪个是可能 commit 的, 哪个是一定没有 commit 的:
> 
> ![](https://mmbiz.qpic.cn/mmbiz_jpg/ziaRl59xuuY8a4UBYuYuGTLibuyUsJGKMVuRQDDq75yj5UtZhbAuIvOicsnQ8RQuCC90hKCG9mrV8gOIvFfT1qSyA/640?wx_fmt=jpeg)
...
例如下面 **例子 3** 中, 如果每个 node 都为其 State 增加一个序号 (在例子中的角标位置), 那么 reader 不论联系到哪 2 个节点, 都可以确定选择序号更大的`[y]` 作为读取结果, 这时就可以认为`[y]` 是一个信息了.

![](https://mmbiz.qpic.cn/mmbiz_jpg/ziaRl59xuuY8a4UBYuYuGTLibuyUsJGKMV4zfgkHicPicY1ibzKl1yKJ13dKwLnkVAPnO92VdNtLNNAll1HzcLib6nDQ/640?wx_fmt=jpeg)

而 **commit 后不能修改** 的原则要求系统所有的修改, 都要基于已 commit 的 State, 所以当系统中再次 commit 一个数据后可能是在`[y]₅` 之上追加`[z,w]`:

![](https://mmbiz.qpic.cn/mmbiz_jpg/ziaRl59xuuY8a4UBYuYuGTLibuyUsJGKMVTxWwxRdqAJrE9zObFxRkVLge86Q5o4FUyicm92kNfKE0w22s7ehxneA/640?wx_fmt=jpeg)
...
![](https://mmbiz.qpic.cn/mmbiz_jpg/ziaRl59xuuY8a4UBYuYuGTLibuyUsJGKMVicQmUV3ibSscL0ibmsxO44Yibmn53GiaYicTrTaY72FahO1Gyll6odbcMVoA/640?wx_fmt=jpeg)

在后面的例子中, 我们将 `commit_index` 写成每条 log 的下标的形式, 例如

![](https://mmbiz.qpic.cn/mmbiz_jpg/ziaRl59xuuY8a4UBYuYuGTLibuyUsJGKMV01XZtJIDMMLJcJXGUudmAWlzS6oia6L2ydVdOEjH4KgBQnwNt4cwgpA/640?wx_fmt=jpeg)

将表示为:

![](https://mmbiz.qpic.cn/mmbiz_jpg/ziaRl59xuuY8a4UBYuYuGTLibuyUsJGKMV3Wq3uPD502tfqwcQtPfAetpC7qH2a0dTJW7zh280AiaHlUmWBtibicG7g/640?wx_fmt=jpeg)

同时定义一个 method 用来取得一个 State 用于比较大小的 commit_index:

![](https://mmbiz.qpic.cn/mmbiz_jpg/ziaRl59xuuY8a4UBYuYuGTLibuyUsJGKMV4pdlyCrNh9UGojuMCdto4DKg5rmicibnuGAlkMULMIM55BPPT3ALxR7g/640?wx_fmt=jpeg)
...
> 另一方面, 从 writer 的角度来说:
> 
> *   如果一个 writer 可以生成一个 `commit_index` 使之大于任何一个已知的 `commit_index`, 那么这时 abstract-paxos 就是一个**活锁**的系统: 它永远不会阻塞, 但有可能永远都不会成功提交. 例如 paxos 或 raft
>     
> *   如果一个 writer 无法生成任意大的 `commit_index`, 那么它就是一个 **死锁** 的系统, 例如 2pc
>     
> 
> 当然也可以构造 `commit_index` 使 abstract-paxos 既活锁又死锁, 那么可以认为它是一个结合了 paxos 和 2pc 的协议.

有了 State 之间的全序关系, 然后再让 writer 保证 **phase-2** 写到 quorum 里的 State 一定是最大的, 进而让 reader 读取时都可以选择这个 State, 达到 **香农信息定义** 要求的信息确定性要求, 即 **commit - 唯一** 的要求, 完成 commit:
...
现在我们来设计整个协议, 首先 有一个 writer `w`, `w` 最终 commit 的操作是在 **phase-2** 将 State 写到一个 quorum. writer 的数据结构定义为一个它选择的 quorum, 以及它决定使用的 `commit_index`:

![](https://mmbiz.qpic.cn/mmbiz_jpg/ziaRl59xuuY8a4UBYuYuGTLibuyUsJGKMVGc6sR5QasSlWRzh7QC5kjFyKPdIbqLwNxVF72Xe9Tjg3VUSvNUusbQ/640?wx_fmt=jpeg)
...
> 例如下面 **例子 5** 中描述的场景, 如果最终写入 State 前不做防御, 那么是无法完成 commit 的: 假设有 2 个 writer `w₁`, `w₂` 同时在写它们自己的 State 到自己的 quorum:
> 
...
> 
> ![](https://mmbiz.qpic.cn/mmbiz_jpg/ziaRl59xuuY8a4UBYuYuGTLibuyUsJGKMVEF37zyONhHCR4OXea1shEvroY4CTnACf6xP7iaPjf2NyQEosP4uMwSA/640?wx_fmt=jpeg)
...
![](https://mmbiz.qpic.cn/mmbiz_jpg/ziaRl59xuuY8a4UBYuYuGTLibuyUsJGKMVXTCQ64qLX0vQ7boZNKC0cu48DjZkNec9s0jBAiawfKdia2GMA8XfET9g/640?wx_fmt=jpeg)

> 在后面的例子中, 我们将用一个数字前缀表示 node 中的 `commit_index`, 例如:
> 
> ![](https://mmbiz.qpic.cn/mmbiz_jpg/ziaRl59xuuY8a4UBYuYuGTLibuyUsJGKMV8GEF04TIkdGBEHBhxklCukY7mZlhTKaYk1s7FWf18hBauWIQzwRnoQ/640?wx_fmt=jpeg)
> 
> 将表示为:
> 
> ![](https://mmbiz.qpic.cn/mmbiz_jpg/ziaRl59xuuY8a4UBYuYuGTLibuyUsJGKMVaYpmDCRjtK9HRb1oMeN9PmtbUqruvefPukwBBlyCic7qmGhzBKwl21A/640?wx_fmt=jpeg)
...
![](https://mmbiz.qpic.cn/mmbiz_jpg/ziaRl59xuuY8a4UBYuYuGTLibuyUsJGKMVTZM9sgC2ib15Hz0Q8wNpYX7zM68BXyjgnLmGZE2xtXtusmib7WPt6rAQ/640?wx_fmt=jpeg)

![](https://mmbiz.qpic.cn/mmbiz_jpg/ziaRl59xuuY8a4UBYuYuGTLibuyUsJGKMV3ApETPX30htqqIPpZfVHHia5GPgqwu3ibOfGg0Iu3RHIdNhh2ibRickCHA/640?wx_fmt=jpeg)
...
![](https://mmbiz.qpic.cn/mmbiz_jpg/ziaRl59xuuY8a4UBYuYuGTLibuyUsJGKMVlicPgLWA5AUIPYrUGqtbtBhZdf521E5uJ39SJabbBAibpKkCEcvKRbXw/640?wx_fmt=jpeg)

### Phase-1: Handler

![](https://mmbiz.qpic.cn/mmbiz_jpg/ziaRl59xuuY8a4UBYuYuGTLibuyUsJGKMVdoQYAxicV66y5FZw9z6aiaQL5YQibRZI149NsD43ZGlhTTmBJKgVwrMqw/640?wx_fmt=jpeg)
...
>     
> *   虽然在 **phase-1** 阶段, 系统已经阻止了所有小于 `s₁.commit_index()` 的其他 State 的 **phase-2** 写入, 如果 `w₁` 写的 State 的 `s_1.commit_index()` 小于`w.commit_index`, 那么系统中可能存在另一个稍大一点的 State (但没有 commit , 导致 reader 不认为 `s₁` 是 commit 的.
>     
> 
> 例如,
> 
...
> 
> ![](https://mmbiz.qpic.cn/mmbiz_jpg/ziaRl59xuuY8a4UBYuYuGTLibuyUsJGKMVFib9ciaf7z1hBXqibw74Jv4DfYNcFr1ib8J0HiaIuJRS9ibtBpW72hdeIUvg/640?wx_fmt=jpeg)
...
![](https://mmbiz.qpic.cn/mmbiz_jpg/ziaRl59xuuY8a4UBYuYuGTLibuyUsJGKMVgRvAGhI1B0evictHgWCRNru9KoriaMRo6eE5s5cz3X677ZUI2qJlvK4A/640?wx_fmt=jpeg)
...
![](https://mmbiz.qpic.cn/mmbiz_jpg/ziaRl59xuuY8a4UBYuYuGTLibuyUsJGKMVJzjNDfffz7YzuvAhBz19iaicIicDibOCjxwwR7CeVKSTDG1QOmydnkMbaA/640?wx_fmt=jpeg)

### Phase-2: Reply

![](https://mmbiz.qpic.cn/mmbiz_jpg/ziaRl59xuuY8a4UBYuYuGTLibuyUsJGKMVicpsf4Boxq3zeHiaxNcibL0oTfN5Man3dKsAib1JD9GEBWYhsNRVx6dLfg/640?wx_fmt=jpeg)

### Phase-2: Handler

![](https://mmbiz.qpic.cn/mmbiz_jpg/ziaRl59xuuY8a4UBYuYuGTLibuyUsJGKMVQvGH88jMibgIuFXhSPQRLcspSrQUMickoq15OQ05Z1TdsyshVBpKu16w/640?wx_fmt=jpeg)

也就是说 **phase-2** 不止可能修改 `Node.state`, 同时也会修改 `Node.commit_index`.

> 这里也是一个学习分布式容易产生误解的地方, 例如很多人曾经以为的一个 paxos 的 bug: paxos-bug.
...
![](https://mmbiz.qpic.cn/mmbiz_jpg/ziaRl59xuuY8a4UBYuYuGTLibuyUsJGKMVNOl8PibqnYLVfxjjVncT66tUY2EDYUrgAGRSuASskuD2JKpvjxiclxHQ/640?wx_fmt=jpeg)
...
![](https://mmbiz.qpic.cn/mmbiz_jpg/ziaRl59xuuY8a4UBYuYuGTLibuyUsJGKMVErnLWmsETUOTIHibunpVoE6WNykSh0w1gicgunMicyLIEhtvvQ5sq6Egg/640?wx_fmt=jpeg)

*   t1 时刻, writer W 联系到 N1, N2 完成 phase-1,, 读到最大的 State `[x₃,z₅]`, 添加自己的日志到最大 State 上: `[x₃,z₅,w₆]`. 这时系统中没有任何一个 node 的 State 是 commit 完成状态的, 一个 reader 可能选择 `[x₃,z₅]` 作为读取结果 (访问 N1,N2), 可能选择 `[x₃,y₄]`作为读取结果 (访问 N2,N3).
...
> 
> ![](https://mmbiz.qpic.cn/mmbiz_jpg/ziaRl59xuuY8a4UBYuYuGTLibuyUsJGKMVSysK89PAeps5iczsF4KA7acNB56VY7ia1supsgW2araycicg83qllVG1w/640?wx_fmt=jpeg)
> 
> 再如一个由 2 个配置组成的 joint config `[{a,b,c}, {x,y,z}]`. 它定义的 quorum 集合是`{a,b,c}` 的 quorum 集合跟 `{x,y,z}`的 guorum 集合的笛卡尔积:
> 
> ![](https://mmbiz.qpic.cn/mmbiz_jpg/ziaRl59xuuY8a4UBYuYuGTLibuyUsJGKMVjCYFLric8NjGFsgwibwibzVv8uxcqzaeTxxIBbVudbCyzSG5jwQV34z1Q/640?wx_fmt=jpeg)
...
![](https://mmbiz.qpic.cn/mmbiz_svg/CJ35Z2cnZA1DyTGhNG3RdcSEI9sI90H61LuAcGZ3uVUE77Al1icQY6UKhl7KuuxnslwDbWQ27VrEMzRSWyenSx5fomu3UWR49/640?wx_fmt=svg)
...
*   **先在旧配置上拒绝更小的 State 的提交, 再 propose 新配置.** 根据 **成员变更 Lemma-1**, 即, 至少将一个与 `w.commit_index` 相同的 State commit 到 `cᵢ` 上.
...
![](https://mmbiz.qpic.cn/mmbiz_jpg/ziaRl59xuuY8a4UBYuYuGTLibuyUsJGKMVeBwbnnDz8G8ew8ssvrVvHWvXEV0ePteiaXzicX8OcM52vdGBHObtzibibw/640?wx_fmt=jpeg)
...
<table width="677"><tbody><tr><th>abstract-paxos</th><th>classic-paxos</th></tr><tr><td data-style="padding: 0.5em; outline: 0px; word-break: break-all; border-bottom-color: rgb(182, 182, 182); max-width: 100%; box-sizing: border-box; transition: all 0.2s ease-in-out 0s; vertical-align: middle; overflow-wrap: break-word !important;">writer</td><td data-style="padding: 0.5em; outline: 0px; word-break: break-all; border-bottom-color: rgb(182, 182, 182); max-width: 100%; box-sizing: border-box; transition: all 0.2s ease-in-out 0s; vertical-align: middle; overflow-wrap: break-word !important;">proposer</td></tr><tr><td data-style="padding: 0.5em; outline: 0px; word-break: break-all; border-bottom-color: rgb(182, 182, 182); max-width: 100%; box-sizing: border-box; transition: all 0.2s ease-in-out 0s; vertical-align: middle; overflow-wrap: break-word !important;">node</td><td data-style="padding: 0.5em; outline: 0px; word-break: break-all; border-bottom-color: rgb(182, 182, 182); max-width: 100%; box-sizing: border-box; transition: all 0.2s ease-in-out 0s; vertical-align: middle; overflow-wrap: break-word !important;">acceptor</td></tr><tr><td data-style="padding: 0.5em; outline: 0px; word-break: break-all; border-bottom-color: rgb(182, 182, 182); max-width: 100%; box-sizing: border-box; transition: all 0.2s ease-in-out 0s; vertical-align: middle; overflow-wrap: break-word !important;">Writer.commit_index</td><td data-style="padding: 0.5em; outline: 0px; word-break: break-all; border-bottom-color: rgb(182, 182, 182); max-width: 100%; box-sizing: border-box; transition: all 0.2s ease-in-out 0s; vertical-align: middle; overflow-wrap: break-word !important;">rnd/ballot</td></tr><tr><td data-style="padding: 0.5em; outline: 0px; word-break: break-all; border-bottom-color: rgb(182, 182, 182); max-width: 100%; box-sizing: border-box; transition: all 0.2s ease-in-out 0s; vertical-align: middle; overflow-wrap: break-word !important;">State.commit_index()</td><td data-style="padding: 0.5em; outline: 0px; word-break: break-all; border-bottom-color: rgb(182, 182, 182); max-width: 100%; box-sizing: border-box; transition: all 0.2s ease-in-out 0s; vertical-align: middle; overflow-wrap: break-word !important;">vrnd/vbal</td></tr></tbody></table>
...
![](https://mmbiz.qpic.cn/mmbiz_jpg/ziaRl59xuuY8a4UBYuYuGTLibuyUsJGKMVXI2wlQn0fK4wiavZBDFFNsOH8EHLbqrvzzLJH9c7lMqbFicHGsPJ0n9w/640?wx_fmt=jpeg)

其中 VotedFor 的大小关系 (即覆盖关系: 大的可以覆盖小的) 定义是:

![](https://mmbiz.qpic.cn/mmbiz_jpg/ziaRl59xuuY8a4UBYuYuGTLibuyUsJGKMV3eXic8jYbXCWSNpRrhwt3zBTAbLl5d0h2XP42PbwAs9RfeNbAlSOgAA/640?wx_fmt=jpeg)

即, VotedFor 只能从 None 变化到 Some, 不能修改. 或者说, `Some(A)` 和 `Some(B)` 没有大小关系, 这限制了 raft 选主时的成功几率. 导致了更多的选主失败冲突.

![](https://mmbiz.qpic.cn/mmbiz_jpg/ziaRl59xuuY8a4UBYuYuGTLibuyUsJGKMVYRPLf8OG3FPMdwBDE8zfJoShMvEaiaCjDcmOkVwrwY0adAJHdXAJmEQ/640?wx_fmt=jpeg)

commit_index 在每条日志中的存储也做了简化, 先看直接嵌入后的结构如下:

![](https://mmbiz.qpic.cn/mmbiz_jpg/ziaRl59xuuY8a4UBYuYuGTLibuyUsJGKMVzQr6P3fKKSC8tePV3KicgISeLg3CxwkUSicibRBiaUUcj9WnkcgKZIHDaw/640?wx_fmt=jpeg)
...
![](https://mmbiz.qpic.cn/mmbiz_jpg/ziaRl59xuuY8a4UBYuYuGTLibuyUsJGKMVMDNPmEBW7ZcBlF7cqiaYOPmGJibCXTZxqXlAYaVCD4hFIkgamIeIWXyQ/640?wx_fmt=jpeg)
...
<table width="677"><tbody><tr><th>abstract-Paxos</th><th>raft</th></tr><tr><td data-style="padding: 0.5em; outline: 0px; word-break: break-all; border-bottom-color: rgb(182, 182, 182); max-width: 100%; box-sizing: border-box; transition: all 0.2s ease-in-out 0s; vertical-align: middle; overflow-wrap: break-word !important;">writer at phase-1</td><td data-style="padding: 0.5em; outline: 0px; word-break: break-all; border-bottom-color: rgb(182, 182, 182); max-width: 100%; box-sizing: border-box; transition: all 0.2s ease-in-out 0s; vertical-align: middle; overflow-wrap: break-word !important;">Candidate</td></tr><tr><td data-style="padding: 0.5em; outline: 0px; word-break: break-all; border-bottom-color: rgb(182, 182, 182); max-width: 100%; box-sizing: border-box; transition: all 0.2s ease-in-out 0s; vertical-align: middle; overflow-wrap: break-word !important;">writer at phase-2</td><td data-style="padding: 0.5em; outline: 0px; word-break: break-all; border-bottom-color: rgb(182, 182, 182); max-width: 100%; box-sizing: border-box; transition: all 0.2s ease-in-out 0s; vertical-align: middle; overflow-wrap: break-word !important;">Leader</td></tr><tr><td data-style="padding: 0.5em; outline: 0px; word-break: break-all; border-bottom-color: rgb(182, 182, 182); max-width: 100%; box-sizing: border-box; transition: all 0.2s ease-in-out 0s; vertical-align: middle; overflow-wrap: break-word !important;">node</td><td data-style="padding: 0.5em; outline: 0px; word-break: break-all; border-bottom-color: rgb(182, 182, 182); max-width: 100%; box-sizing: border-box; transition: all 0.2s ease-in-out 0s; vertical-align: middle; overflow-wrap: break-word !important;">node</td></tr><tr><td data-style="padding: 0.5em; outline: 0px; word-break: break-all; border-bottom-color: rgb(182, 182, 182); max-width: 100%; box-sizing: border-box; transition: all 0.2s ease-in-out 0s; vertical-align: middle; overflow-wrap: break-word !important;">Writer.commit_index</td><td data-style="padding: 0.5em; outline: 0px; word-break: break-all; border-bottom-color: rgb(182, 182, 182); max-width: 100%; box-sizing: border-box; transition: all 0.2s ease-in-out 0s; vertical-align: middle; overflow-wrap: break-word !important;">(Term,VotedFor)</td></tr><tr><td data-style="padding: 0.5em; outline: 0px; word-break: break-all; border-bottom-color: rgb(182, 182, 182); max-width: 100%; box-sizing: border-box; transition: all 0.2s ease-in-out 0s; vertical-align: middle; overflow-wrap: break-word !important;">State.commit_index()</td><td data-style="padding: 0.5em; outline: 0px; word-break: break-all; border-bottom-color: rgb(182, 182, 182); max-width: 100%; box-sizing: border-box; transition: all 0.2s ease-in-out 0s; vertical-align: middle; overflow-wrap: break-word !important;">Term</td></tr></tbody></table>
...
*   1, **一个 term 允许选出多个 leader**: 将 `commit_index` 改为 字典序, 允许一个 term 中先后选出多个 leader.
...
其中 1,3 已经在 openraft 中实现 (朋友说它是披着 raft 皮的 paxos`/:-)`).
